...
constexpr size_t kOutChannelBlock = 10;
  ...
  for (size_t out_y = 0; out_y < out_H; out_y++) {
          ...
          for (size_t w_x = 0; w_x < w_W; w_x++) {
            ...
            if(out_x_start < out_x_end){
              ...
              while (left > 0) {
                ...
                vfloat32m4_t v_in = __riscv_vlse32_v_f32m4(p_start, stride_x * sizeof(float), vl);

                for (size_t k = 0; k < block_size; k++) {
                  vfloat32m4_t v_acc = __riscv_vle32_v_f32m4(&p_out_row[k][out_x], vl);
                  v_acc = __riscv_vfmacc_vf_f32m4(v_acc, kernel_vals[k], v_in, vl);
                  __riscv_vse32_v_f32m4(&p_out_row[k][out_x], v_acc, vl);
    ...
    for (size_t k = 0; k < block_size; k++) {
      if (bias_vals[k] != 0.0f) {
        ...
        while (left > 0) {
          size_t vl = __riscv_vsetvl_e32m4(left);
          vfloat32m4_t v_out = __riscv_vle32_v_f32m4(&p_out_row[k][out_x], vl);
          v_out = __riscv_vfadd_vf_f32m4(v_out, bias_vals[k], vl);
          __riscv_vse32_v_f32m4(&p_out_row[k][out_x], v_out, vl);
